...
class HtmlReport:
    ...
    def generate_html(self,head, file):
            ...
            tab1 << tr(td('用例ID', bgcolor='#ABABAB', align='center')
                       ...
                       + td('系统名称', bgcolor='#ABABAB', align='center')
            ...
            # 查询所有测试结果并记录到html文档
            query = ('SELECT case_id, http_method, request_name,interface_name,test_desc, result,'
                     'reason,request_url, request_param FROM test_result')
            self.cursor.execute(query)
            query_result = self.cursor.fetchall()
            #print("query_result2",query_result)

            for row in query_result:
                tab1<< tr(td(int(row[0]), align='center') + td(row[1]) +
                              td(row[2]) + td(row[3], align='center') +
                              td(row[4],align='center')+ td(row[5]) + td(row[6]) +                   #+ td(row[5])
                              td(row[7], align='center') + td(row[8]))#)   (str(base64.b64decode(row[8]),'utf-8')) str(base64.b64decode(row[8]),'utf-8')
                #print("td(row[8]从数据库查询出来编码处理过的字符串：",td(row[8]))
                #print("td(row[8]解码后结果:",str(base64.b64decode(row[8]),'utf-8'))
